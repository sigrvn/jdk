...
FrankensteinBarrierSet::FrankensteinBarrierSet(G1CardTable* card_table,
                           G1CardTable* refinement_table) :
  CardTableBarrierSet(make_barrier_set_assembler<G1BarrierSetAssembler>(),
                      make_barrier_set_c1<G1BarrierSetC1>(),
                      make_barrier_set_c2<G1BarrierSetC2>(),
                      card_table,
                      BarrierSet::FakeRtti(BarrierSet::FrankensteinBarrierSet)),
  _satb_mark_queue_buffer_allocator("SATB Buffer Allocator", G1SATBBufferSize),
  _satb_mark_queue_set(&_satb_mark_queue_buffer_allocator),
  _refinement_table(refinement_table),
  _barrier_in_use(BarrierSet::G1BarrierSet)
{}

FrankensteinBarrierSet::FrankensteinBarrierSet(PSCardTable* card_table) :
  CardTableBarrierSet(card_table),
  _satb_mark_queue_buffer_allocator("", 0),
  _satb_mark_queue_set(&_satb_mark_queue_buffer_allocator),
  _refinement_table(nullptr),
  _barrier_in_use(BarrierSet::CardTableBarrierSet)
{}

/*
FrankensteinBarrierSet::FrankensteinBarrierSet(CardTableRS* rem_set) :
   CardTableBarrierSet(rem_set),
   _satb_mark_queue_buffer_allocator("", 0),
   _satb_mark_queue_set(&_satb_mark_queue_buffer_allocator),
   _refinement_table(nullptr),
   _barrier_in_use(BarrierSet::CardTableBarrierSet)
...
FrankensteinBarrierSet::~FrankensteinBarrierSet() {
  if (_barrier_in_use == BarrierSet::G1BarrierSet) {
    delete _refinement_table;
  }
}

void FrankensteinBarrierSet::swap_global_card_table() {
...
FrankensteinBarrierSet::write_ref_array_pre_work(T* dst, size_t count) {
  G1SATBMarkQueueSet& queue_set = FrankensteinBarrierSet::satb_mark_queue_set();
...
void FrankensteinBarrierSet::write_ref_array_pre(oop* dst, size_t count, bool dest_uninitialized) {
  if (!dest_uninitialized) {
    write_ref_array_pre_work(dst, count);
  }
}

void FrankensteinBarrierSet::write_ref_array_pre(narrowOop* dst, size_t count, bool dest_uninitialized) {
  if (!dest_uninitialized) {
    write_ref_array_pre_work(dst, count);
  }
}

void FrankensteinBarrierSet::write_region(JavaThread* thread, MemRegion mr) {
  if (mr.is_empty()) {
    return;
  }
...
void FrankensteinBarrierSet::on_thread_create(Thread* thread) {
  // Create thread local data
  G1ThreadLocalData::create(thread);
}

void FrankensteinBarrierSet::on_thread_destroy(Thread* thread) {
  // Destroy thread local data
  G1ThreadLocalData::destroy(thread);
}

void FrankensteinBarrierSet::on_thread_attach(Thread* thread) {
...
void FrankensteinBarrierSet::on_thread_detach(Thread* thread) {
  // Flush any deferred card marks.
  CardTableBarrierSet::on_thread_detach(thread);
  {
    SATBMarkQueue& queue = G1ThreadLocalData::satb_mark_queue(thread);
    FrankensteinBarrierSet::satb_mark_queue_set().flush_queue(queue);
  }
  {
...
void FrankensteinBarrierSet::print_on(outputStream* st) const {
