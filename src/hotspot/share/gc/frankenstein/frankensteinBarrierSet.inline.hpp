...
#ifndef SHARE_GC_FRANKENSTEIN_FRANKENSTEINBARRIERSET_INLINE_HPP
#define SHARE_GC_FRANKENSTEIN_FRANKENSTEINBARRIERSET_INLINE_HPP
...
inline void FrankensteinBarrierSet::enqueue_preloaded(oop pre_val) {
...
template <class T>
inline void FrankensteinBarrierSet::enqueue(T* dst) {
  G1SATBMarkQueueSet& queue_set = FrankensteinBarrierSet::satb_mark_queue_set();
  if (!queue_set.is_active()) return;
...
inline void FrankensteinBarrierSet::write_ref_field_pre(T* field) {
...
inline void FrankensteinBarrierSet::write_region(MemRegion mr) {
  write_region(JavaThread::current(), mr);
}

template <DecoratorSet decorators, typename T>
inline void FrankensteinBarrierSet::write_ref_field_post(T* field) {
...
inline void FrankensteinBarrierSet::enqueue_preloaded_if_weak(DecoratorSet decorators, oop value) {
...
template <typename T>
inline oop FrankensteinBarrierSet::AccessBarrier<decorators, BarrierSetT>::
...
template <DecoratorSet decorators, typename BarrierSetT>
template <typename T>
inline void FrankensteinBarrierSet::AccessBarrier<decorators, BarrierSetT>::
oop_store_not_in_heap(T* addr, oop new_value) {
  ...
  FrankensteinBarrierSet *bs = as_barrier_set<FrankensteinBarrierSet>();
  bs->write_ref_field_pre<decorators>(addr);
  Raw::oop_store(addr, new_value);
}

#endif // SHARE_GC_FRANKENSTEIN_FRANKENSTEINBARRIERSET_INLINE_HPP
