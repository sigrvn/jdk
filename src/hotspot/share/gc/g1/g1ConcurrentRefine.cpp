...
void G1ConcurrentRefineThreadControl::assert_current_thread_is_control_refinement_thread() const {
  assert(Thread::current() == _control_thread, "Not primary thread");
}
#endif // ASSERT
...
bool G1ConcurrentRefineWorkState::sweep_rt_step() {
  ...
  G1ConcurrentRefineWorkTask task(_sweep_state, &_stats, cr->threads_wanted());
...
bool G1ConcurrentRefineWorkState::complete(bool concurrent, bool print_log) {
  ...
  if (print_log) {
    ...
    log_debug(gc, refine)("Refinement took %.2fms (pre-sweep %.2fms card refine %.2f) (scanned %zu clean %zu (%.2f%%) not-clean %zu (%.2f%%) not parsable %zu refers-to-cset %zu (%.2f%%) still refers-to-cset %zu (%.2f%%) clean-again %zu pending %zu)",
...
void G1ConcurrentRefine::run_with_refinement_workers(WorkerTask* task) {
  _thread_control.run_task(task, threads_wanted());
}
...
bool G1ConcurrentRefine::adjust_threads_periodically() {
  ...
  if (_needs_adjust) {
    ...
    if (Heap_lock->try_lock()) {
      ...
      Atomic::store(&_threads_wanted, 0u);
...
bool G1ConcurrentRefine::is_thread_wanted() const {
  return Atomic::load(&_threads_wanted) > 0;
}
