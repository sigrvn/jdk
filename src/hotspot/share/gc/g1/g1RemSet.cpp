...
class G1MergeHeapRootsTask : public WorkerTask {
  ...
  public:
    G1ClearBitmapClosure(G1CollectedHeap* g1h, bool initial_evacuation) : _g1h(g1h), _initial_evacuation(initial_evacuation) { }
    ...
    bool do_heap_region(G1HeapRegion* hr) {
      ...
      // * we use the refinement table for recording reference to other regions
      // during evacuation failure handling
      // * during previous passes we used the refinement table for the to-be card
      // table to record cross-region references
  ...
  virtual void work(uint worker_id) {
...

    {
...

      {
        ...
        G1ClearBitmapClosure clear(g1h, _initial_evacuation);
...
void G1RemSet::merge_heap_roots(bool initial_evacuation) {
  ...
  // 2. (Optionally) Merge the refinement table into the card table
...
G1RemSet::RefineResult G1RemSet::refine_card_concurrently(CardValue* const card_ptr,
  ...
  if (r->oops_on_memregion_seq_iterate_careful<false>(dirty_region, &conc_refine_cl) != nullptr) {
    if (conc_refine_cl.has_to_cset_ref()) {
      return HasToCSetRef;
    } else if (conc_refine_cl.has_to_old_ref_only()) {
      return HasOnlyOld;
    } else {
